...
### Basic Usage:
The `orff` package contains an example passage-level student data set `passage`, which is stored in the `passage.rda` file, to demonstrate some basic usage of the package.     
...
**Step 3:** Run the `wcpm()` function to estimate WCPM scores for selected cases. Here, we enter the manipulated data `datalong` from the Step 1.
```{r eval = F}
...
Alternatively, we can run the `wcpm()` function without **Step 1** above, by entering the original data `passage` as follows.
``` {r eval = F}
sample.cases <- data.frame(cases = c("2056_fall", "2056_winter", "2056_spring"))
test_WCPMEAP <- wcpm(test_MCEM, 
                     stu.data = passage,
