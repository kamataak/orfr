...
**Step 3:** Run the `wcpm()` function to estimate WCPM scores for selected cases. Note that we pass the output object `test_MCEM` from the passage calibration step, as well as the manipulated data `data$data.long` from Step 1. If the `cases =` argument is omitted, WCPM scores will be estimated for all cases in the data.
...
```{r eval = F}
test_WCPMMAP_EXT <- wcpm(calib.data=test_MCEM, 
                     stu.data = data$data.long,
                     cases = sample.cases, 
                     external = c("22007","22013","22036","22043","22048","22079"),
                     est = "map", 
                     se = "analytic")
summary(test_WCPMMAP_EXT)
```
...
``` {r eval = F}
test_WCPMMAP <- wcpm(calib.data=test_MCEM, 
                     stu.data = passage,
                     studentid = "id.student",
                     passageid = "id.passage",
                     season = "occasion",
