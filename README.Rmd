...
By default, the standard errors for the model parameters are not estimated. This will allow one to increase the number of Monte-Carlo iteration `reps.in` to improve the quality of the estimates of the model parameters, while minimizing the computation time. The number of `reps.in` should be 50-100 in realistic calibrations. SE's for model parameters are not required for running `wcpm()` function to estimate WCPM scores in the next step. In order to compute the standard errors for the model parameters, an additional argument `se = "analytical"` or `se = "bootstrap"` needs to be added to the `mcem()` function. 
