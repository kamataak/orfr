...
)
{
  ...
  for(i in 1:n.chains){
    ...
    gen_init <- list(a=a, b_reg=b_reg, alpha=alpha, beta_raw=beta_raw, cvr=cvr, ptau=ptau,
...
# J students and I passages
    for (j in 1:J) {
      for (i in 1:I) {
        ...
        probit(p[j,i]) <- a[i] * theta[j] + b_reg[i]
...
parameters{
  ...
  vector[I] b_reg; //accuracy difficulty (threshold style)
...
transformed parameters{
  ...
  vector[I] b; //accuracy difficulty (IRT style)
...
// Likelihood
  for(i in 1:I){
  res[i] ~ binomial(nw[i], Phi(a[i] * theta + b_reg[i]));
